...
<div class="container">
	<div class="row">
		<c:forEach items="${theme_onePageList}" var="list">
			...
			<td colspan="3" height="100px">
				<!-- 페이지 카운트 --> <c:if test="${count>0}">
					...
					<c:if test="${startPage>5}">
						<a href="theme_list?pageNum=${startPage-5}">[이전]</a>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${endPage}">

						<c:if test="${categoryValue=='no'}">
							<a href="theme_list?pageNum=${i}">[${i}]</a>
						</c:if>
						<c:if test="${categoryValue!='no'}">
							<a href="theme_list?pageNum=${i}&p_category2=${categoryValue}">[${i}]</a>
						</c:if>

					</c:forEach>
					<c:if test="${endPage<pageCount}">
						<a href="theme_list?pageNum=${startPage+5}">[다음]</a>
					</c:if>
				</c:if>
			</td>
		</tr>
