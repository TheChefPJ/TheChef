						...
						<div>
							...
							<div>
								<div class=""
									...
									<input type="text" name="dataForSearch" placeholder="이벤트 제목을 입력하세요." style="width:300px">
								</div>
		         ...
		         <table width="100%">
		            <tr>
		               <td align="center">
		                  <!-- 처음 이전 링크 --> 
		                  <c:if test="${pg>block}">
		                     <a href="EB_eventManage?pg=${fromPage-1}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}"><img
		                        src="<c:url value='resources/img/arrowsleft.gif'/>"></a>
		                     <a href="EB_eventManage?pg=${pg-1}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}"><img
		                        src="<c:url value='resources/img/arrowleft.gif'/>"></a>
		                  </c:if> 
		                  <c:if test="${pg<=block}">
		                      <c:if test="${pg>1}">
		                           <img src="<c:url value='resources/img/arrowsleft.gif'/>">
		                           <a href="EB_eventManage?pg=${pg-1}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}">
		                           <img
		                          src="<c:url value='resources/img/arrowleft.gif'/>"></a>
		                      </c:if>   
		                  ...
		                  <c:forEach begin="${fromPage}" end="${toPage}"
		                     ...
		                     <c:if test="${i!=pg}">
		                        <a href="EB_eventManage?pg=${i}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}">${i}</a>
		                     </c:if>
		                  </c:forEach>  
		                  <!-- 다음, 이후 --> 
		                  <c:if test="${toPage<allPage}">
		                     ...
		                     <a href="EB_eventManage?pg=${toPage+1}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}"><img
